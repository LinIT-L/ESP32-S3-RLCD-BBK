...
/*
 * gnuboy_deinit releases the heap allocations made by gnuboy_init (via
 * gb_hw_init). Switching emulators / exiting the GBC menu leaks one set of
 * rambanks/vbanks per gnuboy_init call otherwise; over time PSRAM fills up
 * and other engines (e.g. gam4980's 6MB) fail to allocate. Called by
 * gbc_emu_unload.
 */
