...
next:
	...
	div_advance(clen);
	timer_advance(clen);
	/* V1.0.61: 声音累加必须用完整周期 (APU 时钟 2MHz 与 CPU 双速无关).
	 * 原代码先 clen >>= cpu->speed 再 sound_advance, CGB 双速 (speed=1)
	 * 时每帧只累加 17556/35112 周期 → 音频只产 367 样本/帧 → 重采样拉伸
	 * 2 倍 → GBC 声音慢一倍. cpu_idle 路径 sound_advance(cnt) 用的是完整
	 * cnt, 两条路径不一致, 这里统一为完整周期. */
	sound_advance(clen);
	clen >>= cpu->speed;
	lcdc_advance(clen);
