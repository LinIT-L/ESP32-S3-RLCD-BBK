...
/*
 * gb_hw_deinit frees the heap allocations made by gb_hw_init.
 * gnuboy 核心在 gb_hw_init 里分配了 rambanks/vbanks (放 PSRAM), 切换模拟器/退出
 * GBC 菜单时若不释放, 每次 gnuboy_init 都会新增一份, PSRAM 会越积越满最终导致
 * 其他引擎 (如 gam4980 的 6MB) 分配失败. 由 gbc_emu_unload -> gnuboy_deinit 调用.
 */
void gb_hw_deinit(void)
{
	if (hw.rambanks)
	{
		free(hw.rambanks);
	...
	/* hw.snd / hw.cpu 是静态对象 (gb_sound_init/gb_cpu_init 返回 &snd/&cpu), 无需释放 */
}
