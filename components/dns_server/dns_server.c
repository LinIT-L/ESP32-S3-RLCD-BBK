...
struct dns_server_handle {
    bool started;
    TaskHandle_t task;
    volatile int sock;   /* V1.0.67: 记录 socket, stop 时先 shutdown 释放 53 端口 */
    int num_of_entries;
...
void dns_server_task(void *pvParameters)
{
    ...
    while (handle->started) {
        ...
        handle->sock = sock;   /* V1.0.67: 记录, stop 时先 shutdown 释放端口 */
        ...
        while (handle->started) {
            ...
            // Error occurred during receiving (含 stop 时 shutdown 唤醒)
            if (len < 0) {
                break;
            }
            // Data received
    ...
    free(handle);   /* V1.0.67: 任务退出时自行释放 handle, 避免 stop 里 use-after-free */
    vTaskDelete(NULL);
}
...
void stop_dns_server(dns_server_handle_t handle)
{
    if (handle) {
        ...
        /* V1.0.67: 只 shutdown 唤醒 recvfrom 并释放 53 端口;
         * 任务被唤醒后自行 close socket + free(handle) + 退出, 避免 use-after-free 死机 */
        if (handle->sock >= 0) {
