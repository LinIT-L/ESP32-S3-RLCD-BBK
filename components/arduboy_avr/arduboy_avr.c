 ...
 * simavr 的 s_avr->data 是 AVR SRAM (含寄存器区), 游戏变量都在这里.
 * 只经 s_avr->data peek/poke, 绝不触碰其它系统/软件内存. */
static uint8_t arduboy_cheat_peek(uint16_t off)
{
    if (!s_avr || !s_avr->data) return 0;
    uint16_t n = (uint16_t)(s_avr->ramend + 1);
    return (off < n) ? s_avr->data[off] : 0;
}
static void arduboy_cheat_poke(uint16_t off, uint8_t v)
{
    if (!s_avr || !s_avr->data) return;
    uint16_t n = (uint16_t)(s_avr->ramend + 1);
    if (off < n) s_avr->data[off] = v;
}
static cheater_region_t arduboy_cheat_region = {
    .base = 0,
    .size = 0x1000,     /* 上限; peek/poke 内部按 ramend+1 钳制 */
    .peek = arduboy_cheat_peek,
    .poke = arduboy_cheat_poke,
};
...
esp_err_t arduboy_avr_start(const char *path)
{
    ...
    s_joypad = 0xFF;
    arduboy_avr_update_buttons();
    /* 独立修改机浮标: 登记本局游戏内存 region */
    cheat_float_attach(&arduboy_cheat_region, CHEAT_BITS_8);
    ssd1306_set_flag(&s_disp, SSD1306_FLAG_DIRTY, 1);
    s_last_render_ms = 0;
