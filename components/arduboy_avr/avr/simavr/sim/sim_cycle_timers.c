...
/* 性能剖析: 累计 avr_cycle_timer_process 占用的主机周期 (仅诊断用).
 * 每批两次 esp_cpu_get_cycle_count() 开销可忽略. */
#if defined(__XTENSA__) || defined(CONFIG_IDF_TARGET_ESP32S3)
static uint64_t s_prof_timer_cycles = 0;
static uint32_t s_prof_timer_calls = 0;
uint64_t sim_prof_timer_cycles(void) { return s_prof_timer_cycles; }
uint32_t sim_prof_timer_calls(void) { return s_prof_timer_calls; }
void sim_prof_timer_reset(void) { s_prof_timer_cycles = 0; s_prof_timer_calls = 0; }
#endif
 ...
 */
avr_cycle_count_t
AVR_IRAM avr_cycle_timer_process(
		avr_t * avr)
{
	avr_cycle_timer_pool_t * pool = &avr->cycle_timers;
#if defined(__XTENSA__) || defined(CONFIG_IDF_TARGET_ESP32S3)
...
#if defined(__XTENSA__) || defined(CONFIG_IDF_TARGET_ESP32S3)
	{
		const uint32_t prof_dt = esp_cpu_get_cycle_count() - prof_t0;
		s_prof_timer_cycles += prof_dt;
		s_prof_timer_calls++;
	}
#endif
