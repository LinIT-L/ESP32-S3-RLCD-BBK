...
/* 性能关键: 把 AVR 解释器热路径放进 IRAM (SRAM) 执行.
 * avr_run_one 是一个 ~30KB 的巨型 switch, 从 flash 执行会持续击穿指令缓存,
 * 导致每个 AVR 指令平均 ~120 主机周期, 吞吐被锁死在 ~2MHz.
 * 放到 IRAM 后绕过 XIP 缓存抖动, 吞吐可提升数倍. */
...
void AVR_IRAM avr_core_watch_write(avr_t *avr, uint16_t addr, uint8_t v)
{
	if (addr > avr->ramend) {
...
uint8_t AVR_IRAM avr_core_watch_read(avr_t *avr, uint16_t addr)
{
	if (addr > avr->ramend) {
		uint16_t ramstart = avr->ioend + 1;
		uint16_t ramsize = avr->ramend - ramstart + 1;
 ...
 * entirely accurate.
 */
avr_flashaddr_t AVR_IRAM avr_run_one(avr_t * avr)
{
	/* 本地批处理预算: 进入时快照一次. 游戏频繁写定时器寄存器 (OCR/TCNT)
	 * 会触发 avr_cycle_timer_register/cancel, 把 avr->run_cycle_count 改写为
	 * 下一次定时器到期距离 (可小至几十周期), 导致每批只执行几十条指令,
	 * 批间开销把吞吐锁死在 ~2MHz. 本地快照后, 中途定时器操作不再影响
	 * 当前批处理; 下一批预算仍由批结束后的 avr_cycle_timer_process 精确计算. */
	avr_cycle_count_t run_cycle_count = avr->run_cycle_count;
	/* 中断批量服务计数器: 每 128 条指令服务一次待处理中断, 而不是每次
	 * avr_raise_interrupt 都立刻打断批处理. 游戏显示代码逐字节写 SPI 并
	 * 轮询 SPIF, 若每个字节都打断批处理, 批间开销会把吞吐锁死在 ~2MHz. */
	uint32_t service_counter = 0;
...
//		avr->pc >= avr->codeend, _avr_sp_get(avr) > avr->ramend, avr->pc);
	...
	if (avr->state == cpu_Running) {
		if (avr->interrupt_state && ++service_counter >= 128) {
			service_counter = 0;
			/* 中断返回地址 = 下一条指令 (与 simavr 批结束时服务语义一致) */
			avr->pc = new_pc;
			avr_service_interrupts(avr);
			if (avr->pc != new_pc) {
				new_pc = avr->pc;  /* 已跳入 ISR, 从 ISR 向量继续执行 */
			} else {
				avr->pc = new_pc;  /* 未服务 (如 I=0), 保持原状 */
			}
		}
		if (run_cycle_count > cycle) {
			run_cycle_count -= cycle;
			avr->pc = new_pc;
			goto run_one_again;
		}
	}
