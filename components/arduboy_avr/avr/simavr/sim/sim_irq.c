...
int
avr_irq_has_observers(
		avr_irq_t * irq)
{
	avr_irq_hook_t * hook = irq ? irq->hook : NULL;
	int depth = 0;
	while (hook && depth++ < 16) {
		/* 跳过 ioport/extint 的内部回调: 前者仅供读取, 后者按 EIMSK/ISC
		 * 惰性判定, 都不是"无条件可观测"监听者 (输出引脚常被它们误挂). */
		if (hook->notify && hook->notify != avr_ioport_irq_notify &&
			hook->notify != avr_extint_irq_notify)
			return 1;
		if (hook->chain && avr_irq_has_observers(hook->chain))
			return 1;
		hook = hook->next;
	}
	return 0;
