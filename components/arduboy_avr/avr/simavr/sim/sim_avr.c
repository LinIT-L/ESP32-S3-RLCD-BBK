		...
		avr_t * avr)
{
	...
	/* 释放 avr_init 分配的其余资源, 避免每次 加载/卸载 泄漏:
	 * - 核心 IRQ (AVR_CORE_IRQ_COUNT=1, avr_alloc_irq 在 PSRAM)
	 * - irq 池数组 (pool->irq 由 _avr_irq_pool_add realloc 而来)
	 * - data_names (calloc(ioend+1)) */
	if (avr->irq) {
		avr_free_irq(avr->irq, AVR_CORE_IRQ_COUNT);
		avr->irq = NULL;
	}
	if (avr->irq_pool.irq) {
		free(avr->irq_pool.irq);
		avr->irq_pool.irq = NULL;
		avr->irq_pool.count = 0;
	}
	if (avr->data_names) {
		free(avr->data_names);
		avr->data_names = NULL;
