...
#include "esp_attr.h"
#define AVR_IRAM IRAM_ATTR
#else
#define AVR_IRAM
#endif

/*
 * 速度倍率: 游戏帧周期由 tov_cycles = 预分频 * (top+1) 决定 (AVR 指令数),
 * 与 CPU 频率无关. ESP32 上真实 AVR 吞吐只有 ~1.7MHz, 导致游戏只有 ~10FPS.
 * 把定时器周期按此倍率缩短, 让游戏在相同指令吞吐下推进更快 -- 等效于"频率 xN".
 * 同时影响音频翻转频率, 所以音频频率计算时也要除以该倍率补偿.
 */
uint32_t simavr_speed_factor = 1;

void avr_timer_set_speed_factor(uint32_t f)
{
	simavr_speed_factor = (f < 1) ? 1 : f;
}
		...
		uint8_t raise_interrupt)
{
	...
	switch (mode) {
		...
		case avr_timer_com_toggle: // Toggle OCnA on compare match
			/* 无观察者时跳过引脚级 IRQ 传播: 中断标志已由上方
			 * avr_raise_interrupt 置位, 游戏逻辑不受影响; 音频 hook
			 * 存在时视为观察者, 翻转计数照常工作. */
			if (avr_irq_has_observers(irq)) {
				if (p->comp[comp].com_pin.reg)	// we got a physical pin
					avr_raise_irq(irq,
							flags |
...
static avr_cycle_count_t AVR_IRAM
avr_timer_compa(
		struct avr_t * avr,
		avr_cycle_count_t when,
		void * param)
		...
		void * param)
{
	...
	if (getenv("ABDBG")) printf("[tov] timer%c start=%d when=%llu cyc=%llu tov_cycles=%u tov_top=%u\n",
		p->name, start, (unsigned long long)when, (unsigned long long)avr->cycle,
		(unsigned)p->tov_cycles, (unsigned)p->tov_top);
	...
	if (!start)
		avr_raise_interrupt(avr, &p->overflow);
	/* 关键修复: 批处理变长后定时器可能"迟到"触发 (when < avr->cycle).
	 * 必须把 tov_base 记为实际触发周期 avr->cycle, 否则游戏在触发后重配
	 * 定时器时, reconfigure 用 tov_cycles-(cycle-tov_base) 算出极小/负的
	 * 剩余周期, tov 立刻再触发 → 中断风暴 → 吞吐锁死在 ~2MHz. */
	p->tov_base = avr->cycle;
		...
		uint8_t reset)
{
	...
	avr_t * avr = p->io.avr;
	if (getenv("ABDBG")) printf("[cfg] timer%c prescaler=%u top=%u reset=%d\n", p->name, prescaler, top, reset);
	...
	if (!use_ext_clock) {
		...
		p->tov_cycles = (prescaler * (top+1)) / simavr_speed_factor;
		p->tov_cycles_fract = 0.0f;
	...
	for (int compi = 0; compi < AVR_TIMER_COMP_COUNT; compi++) {
		...
		if (virt_ext_clock)
			...
			comp_cycles = (prescaler * (ocr + 1)) / simavr_speed_factor;
		...
		void * param)
{
	...
	switch (timer->wgm_op_mode_kind) {
		...
		case avr_timer_wgm_ctc:
			/* 写相同值不触发重配置 (与 fast_pwm 分支一致), 避免
			 * 取消/重挂整组 cycle timers 的 O(n) 开销 */
