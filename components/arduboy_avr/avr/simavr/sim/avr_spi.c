		...
		void * param)
{
	...
	if (addr == p->r_spdr) {
		...
		/* 性能优化: 主机模式 SPI 字节传输同步完成 (SPIF 立即置位).
		 * 原实现每个字节注册一个周期定时器, 传输完成后触发 SPI 中断,
		 * 把解释器批处理打断成几十周期一段 (吞吐锁死在 ~2MHz).
		 * 游戏 (如格斗零) 是轮询 SPSR.SPIF 等待传输完成的, 同步完成后
		 * 轮询立即通过, 显示循环以解释器原生速度运行. SSD1306 虚拟屏
		 * 不依赖字节传输耗时, 无时序副作用. */
		if (avr_regbit_get(avr, p->spe) && avr_regbit_get(avr, p->mstr))
			avr_spi_raise(avr, avr->cycle, p);
	}
}
