...
EXT_RAM_BSS_ATTR struct TASK task[LAVA_TASK_COUNT]; //����ջ
int task_lev; //����
byte *lRam;
byte eval_top;
byte secret;
...
/* 调试: 最近执行的指令地址环 (供 pc_platform 冻结分析) */
#define DBG_PC_N 1024
EXT_RAM_BSS_ATTR size_t dbg_pcs[DBG_PC_N];   /* 调试指令地址环: 放 PSRAM 省内部 RAM */
int    dbg_pc_idx = 0;
...
{
	/* V1.0.9x 黑屏修复: 原实现调用 LavaRuntimeError 并置 lPC=NULL, 使 LavaX OS
	 * (Shell.sys) 在正常越界访问时直接终止主循环 → 真机黑屏/闪退。
	 * 对齐原版 LavaXVM (CheckAddr + CHECKRAM=0) 行为: 越界仅打印日志(限流),
	 * 读返回 0 / 写忽略, 程序继续运行。 */
...
extern size_t g_vm_ram_size;   /* 实际分配的 VM RAM 大小 (V1.0.9x) */
...
static byte *vm_memory_pointer(a32 addr,size_t size,const char *operation)
{
	...
	size_t offset;
	size_t available;
	size_t ram_size = g_vm_ram_size ? g_vm_ram_size : MY_DATA_SIZE;

	if (!VRam || !lRam || current<base || current-base>ram_size) {
		vm_memory_error(addr,size,operation);
		return NULL;
	}
	offset=(size_t)(current-base);
	available=ram_size-offset;
...
static char *vm_cstring_pointer(a32 addr,const char *operation)
{
	...
	available=(g_vm_ram_size ? g_vm_ram_size : MY_DATA_SIZE)-task_offset-(size_t)addr;
...
{
	/* V1.0.9x 黑屏修复: 对齐原版 LavaXVM (跳转不校验范围). 原校验使
	 * Shell.sys 正常跳转越界时置 lPC=NULL → 主循环退出 → 真机黑屏. */
	lPC=pLAVA+offset;
	return 1;
}
...
byte getcode()
{
	/* V1.0.9x 黑屏修复: 对齐原版 LavaXVM (无检查, 直接 *(lPC++)). 原
	 * code_pointer_valid 校验拦截 Shell.sys 正常字节流读取 → 黑屏. */
	return *(lPC++);
}
...
{
	a32 t;
	// ע�⣺��������벻�ɺϲ�ΪlRam[eval_stack+eval_top++]=lRamRead(get_gaddr())
	// ��Ϊget_gaddr()��ı�eval_top������ʽ����õ���eval_top
	// ����nds�������Ĳ�ǡ���Ż�����ʹ��eval_top���ձ�Ԥ��ֵ��4���Ӷ��������ջ��ƽ��
	// ���ǣ���gba�ϱ���ȴ����ȷ�ģ�vc��Ҳ��ȷ
	t=get_gaddr();
	lRam[eval_stack+eval_top++]=lRamRead(t);
	lRam[eval_stack+eval_top++]=0;
	lRam[eval_stack+eval_top++]=0;
...
void inc_dec_com()
{
	...
	if (RamBits>16) {
		t=(a3>>24)&0x7f;
		if (a3&0x80000000) a3+=local_bp; //???�п���Խ�磬��Ҳ���Բ�����
		a3&=0xffffff;
		...
		t=(a3>>16)&0x7f;
		if (a3&0x800000) a3+=local_bp; //???�п���Խ�磬��Ҳ���Բ�����
		a3&=0xffff;
		if (t==1) a1=lRamRead(a3);
		else if (t==2) a1=lRamRead2(a3);
		else a1=lRamRead4(a3);
...
void c_let()
{
	...
	get_vals();
	if (RamBits>16) {
		if (a1&0x80000000) t1=(a1+local_bp)&0xffffff; //???�п���Խ�磬��Ҳ���Բ�����
		else t1=a1&0xffffff;
		t=(a1>>24)&0x7f;
	} else {
		if (a1&0x800000) t1=(a1+local_bp)&0xffff; //???�п���Խ�磬��Ҳ���Բ�����
		else t1=a1&0xffff;
		t=(a1>>16)&0x7f;
	}
...
void set_sp()
{
	...
	//if (local_sp&3) local_sp+=4-(local_sp&3); //���ջ��ʼ��4�ֽڱ߽�
}
...
void put_dot()
{
	if (xx>=ScreenWidth) return; //��pc���������������У��Ͼ���������LCD����ICON��
...
static void write81(word x,word y,word width,word height,byte *data)
{
	...
	if (y>=ScreenHeight) {
		temp=0x10000-y;
		if (height<=temp) return; //ȫ�ڻ����⣬����Ҫ��
		height-=temp;
		td+=temp*widths;
		y=0;
	}
	if (y+height>ScreenHeight) height=ScreenHeight-y; //��ȥ����������
	if (x_bak>=ScreenWidth) {
		temp=0x10000-x_bak;
		if (width<=temp) return; //ȫ�ڻ����⣬����Ҫ��
		width-=temp;
		td+=temp;
		x_bak=0;
	}
...
{
	word t;
	if (Y0>=ScreenHeight) return 0; //������Ļ��
	if (x0>x1) {
		t=x0;
		x0=x1;
		x1=t;
	}
	if (x0>=ScreenWidth) return 0; //������Ļ��
	if (x1>=ScreenWidth) x1=ScreenWidth-1;
	return 1;
}
...
void update_lcd_small() //Test û�п��Ƿ���
{
	byte c,c2;
	unsigned long mask;
	int i,j,color;
...
void update_lcd_large() //Test û�п��Ƿ���
{
	byte c,c2;
	unsigned long mask;
	int i,j;
...
void printfloat()
{
	...
	for (i=0;;i++) {
		if (num[i]=='e' && i) flag=1;
		if (flag) flag++;
		if (flag==4) continue; //ת��x.xxxxxe+0yyΪx.xxxxxe+yy
		if (num[i]) cout(num[i]);
		else break;
	}
}
...
void c_getblock()
{
	...
	else xx=(word)(a1&0xfff8); //ʹxx��8��������
	if (width==0 || height==0 || xx>=ScreenWidth || yy>=ScreenHeight) return; //�ݴ�
...
void main_loop()
{
	...
	for (;;) {
		lavax_platform_poll();
		if (CheckExitKey()) {
			printf("[LAVAX-EXIT] CheckExitKey\n"); return;
		}
		CheckAppExit();
		current_opcode_offset=lPC && pLAVA ? (size_t)(lPC-pLAVA) : 0;
		current_opcode=-1;
		t=getcode();
		dbg_pcs[dbg_pc_idx] = current_opcode_offset; dbg_pc_idx = (dbg_pc_idx + 1) % DBG_PC_N; /* 记录指令地址环 */
		if (!lPC) { printf("[LAVAX-EXIT] lPC==NULL before getcode off=0x%lX\n",(unsigned long)current_opcode_offset); return; }
		current_opcode=t;
		if (t&0x80) {
			if ((size_t)(t&0x7f)>=sizeof(codes2)/sizeof(codes2[0])) {
				LavaRuntimeError("LavaXVM system opcode is invalid");
				printf("[LAVAX-EXIT] bad sys opcode 0x%02X off=0x%lX\n",(unsigned int)(t&0x7f),(unsigned long)current_opcode_offset); return;
			}
			code=codes2[t&0x7f];
			(*code)();
		} else {
			if ((size_t)t>=sizeof(codes)/sizeof(codes[0])) {
				...
				printf("[LAVAX-EXIT] bad opcode %d off=0x%lX\n",t,(unsigned long)current_opcode_offset); return;
			}
			code=codes[t];
			(*code)();
		}
		if (!lPC) { printf("[LAVAX-EXIT] lPC==NULL after code off=0x%lX opcode=%d\n",(unsigned long)current_opcode_offset,current_opcode); return; }
	}
}
