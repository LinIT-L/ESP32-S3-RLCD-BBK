...
/* V1.0.9x: 平台触摸钩子 — PC 模拟器提供强符号 (鼠标点击→触摸);
 * 固件未实现则为 weak 空实现 (与原先 GetTouch 置空行为一致). */
__attribute__((weak)) void lavax_platform_get_touch(struct TOUCH *touch)
{
    (void)touch;
}
...
void GetTouch(struct TOUCH *touch)
{
    ...
    lavax_platform_get_touch(touch);
}
