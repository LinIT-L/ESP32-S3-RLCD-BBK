...
const char ProgramRoot[]="fat:/lava/PROGRAM/";
const char ProgramRootSD[]="/sd/lava/PROGRAM/";
const char ProgramRootVirtual[]="/lava/PROGRAM/";

FILE *fhandle[MAX_OPEN_FILE];
byte ftask[MAX_OPEN_FILE];

char FileName[I_MAX_PATH];
char CD[I_MAX_PATH]; //���⵱ǰ·��(��β����'/'),RootPath��CD���ӳ�ΪĿ�������ʵ·��
char RootPath[I_MAX_PATH]; //Lvc����ĸ�Ŀ¼(��β��'/')
...
static void true_name_0(char *src,char *obj,char *root)
{
	...
	strcpy(obj,root);
	if (strlcat(obj,src,I_MAX_PATH)>=I_MAX_PATH) { //��ֹ�������������
		obj[0]=0;
		return;
	}
	for (;;) {
...
void c_fopen()
{
	...
	if (fp==NULL) {
		/* V1.x 缺文件诊断: 打印游戏请求的文件名与最终落盘路径, 用于定位"提示少文件" */
		{
			char dh[I_MAX_PATH];
			const char *dhp = lavax_host_path(FileName, dh, sizeof(dh));
			printf("[LAVAX-FILE] c_fopen FAIL: name<'%s'> hostPath<%s>\r\n",
				FileName, dhp ? dhp : "NULL");
		}
		a1=0;
		put_val();
		return; //ļ޷
...
void c_closeall()
{
	...
	for (i=0;i<MAX_OPEN_FILE;i++) {
		if (fhandle[i]) {
			if (ftask[i]==task_lev) { //ֻ�رյ�ǰ����򿪵��ļ�
				fclose(fhandle[i]);
				fhandle[i]=NULL;
			}
		}
