...
/* 游戏速度倍速: 硬件 tick 每秒 256 次, 每次推进该值个内部 tick.
 * 默认 8 (×8, 同步步高 timer_rate≈8). 供设置/引擎优化页读写覆盖 (0 则按原速). */
extern int lavax_game_speed;
...
/* V1.0.9x: 修改机热键检测 + 呼出共享UI (由 lavax_platform_poll 每帧调用).
 * 内部仅在按下 SELECT+START 时呼出, 阻塞在 cheat_ui_open 期间 VM 停跑(游戏冻结). */
...
/* 显示模式 (引擎优化 s_edm): 0=点对点 1=全屏 2=拉伸. 加载前调用. */
void lavax_set_display_mode(int mode);
