...
static void init(uint8_t *romdata, size_t rom_data_size) {
  ...
  if (!sound_q) {
    printf("Failed to allocate Genesis sound queue\n");
    deinit_genesis();
    return;
  }
#endif
  ...
  if (!gwenesis_sn76489_buffer || !gwenesis_ym2612_buffer || !M68K_RAM || !lfo_pm_table ||
    ...
    printf("Failed to allocate Genesis runtime memory\n");
    deinit_genesis();
    return;
  }
  ...
  if (!genesis_sound_task_create()) {
    printf("Failed to create Genesis sound task\n");
    deinit_genesis();
    return;
  }
#endif
