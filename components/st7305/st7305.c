...
esp_err_t st7305_init(st7305_handle_t *out, const st7305_config_t *config) {
    ...
    *out = (st7305_handle_t){
    ...
    /* 20260812: 帧缓冲改回内部 RAM 优先.
     * 游戏时模拟器在 core1 持续读写 PSRAM, SPI DMA 从 PSRAM 读整帧会被抢带宽,
     * 写入被拖慢且节奏抖动 -> 面板闪烁 (菜单/测试无模拟器抢带宽所以不闪).
     * g_menu 已移到 PSRAM (腾出 ~33KB), 15KB 内部帧缓冲可负担. */
    out->fb = (uint8_t *)heap_caps_calloc(1, FB_BYTES, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (!out->fb) {
        ESP_LOGW(TAG, "SRAM full, fallback to PSRAM");
