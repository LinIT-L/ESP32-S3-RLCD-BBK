...
/* 返回当前 st7305 句柄 (供修改机共享UI等取实时帧缓冲; 未设置返回 NULL) */
...
/* === 屏保独占 LCD (V1.5.x): 屏保激活时冻结游戏刷新, 让壁纸始终画在最上层 ===
 * 所有游戏/引擎的帧最终都经 core0 视频刷新任务 st7305_flush_from 刷屏;
 * active=true 时该任务不再刷游戏帧 → 星空壁纸不再被引擎覆盖.
 * 由壁纸模块在进入/退出屏保时设置. */
void board_rlcd_screensaver_set(bool active);
