...
static const char s_index_html[] =
    ...
    "<title>ESP32-BBK \xe6\x89\x8b\xe6\x9f\x84</title>"
    "<style>"
    ...
    ".fs{position:absolute;top:2vh;right:3vw;height:46px;padding:0 16px;font-size:15px;border-radius:23px;background:#333;border-color:#888;display:none;z-index:10}"
    "/* æ¨ªå±æ¯æ: ææºæ¨ªææ¶æ§å¶å¨è·ééæ (V1.1.1) */"
    "@media (orientation:landscape){"
    ".dpad{left:3vw;bottom:3vh;grid-template-columns:48px 48px 48px;grid-template-rows:48px 48px 48px;gap:4px}"
    ".dpad .btn{font-size:17px}"
    ".stick{left:5vw;bottom:3vh;width:140px;height:140px}"
    ".ab{right:3vw;bottom:3vh;width:130px;height:130px}"
    ".ab .btn{width:58px;height:58px;font-size:20px}"
    ".sys{top:2vh;bottom:auto}"
    "#st{top:9vh;bottom:auto}"
    ".btn{font-size:20px}"
    ".fs{display:flex}"
    "}"
    "<div class='wrap'>"
    ...
    "</div>"
    "<div class='btn fs' id='fs'>\xe5\x85\xa8\xe5\xb1\x8f</div>"
    ...
    "fs.addEventListener('pointerdown',e=>{e.preventDefault();vib();"
