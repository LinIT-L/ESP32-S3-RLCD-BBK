...
volatile uint32_t nes_apu_write_count = 0;   /* 诊断 */
...
/* 诊断: 输出当前 APU 状态, 供 video_audio.c 周期打印 */
void nes_apu_dump_state(char *buf, size_t len)
{
   if (!apu || !buf || len < 8)
   {
      if (buf && len) buf[0] = 0;
   ...
   snprintf(buf, len,
            "mix=%02X en=%02X r0e=%d r0v=%d r0o=%d r0f=%d trie=%d triv=%d triout=%d noe=%d nov=%d noout=%d dmc=%d",
            (unsigned)apu->mix_enable, (unsigned)apu->enable_reg,
            apu->rectangle[0].enabled ? 1 : 0, apu->rectangle[0].vbl_length,
            apu->rectangle[0].output_vol, apu->rectangle[0].freq,
            apu->triangle.enabled ? 1 : 0, apu->triangle.vbl_length,
            apu->triangle.output_vol,
            apu->noise.enabled ? 1 : 0, apu->noise.vbl_length,
