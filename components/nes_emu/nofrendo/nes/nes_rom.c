...
rominfo_t *nes_rom_load(const char *filename)
{
   ...
   */
   ...
   /* V1.0.53: 原生 rom_loadsram 会从 ROM 文件本身读 SRAM 大小的数据
    * (osd_newextension 被注释), 得到的是 ROM 头部垃圾. 存档由 nes_emu 适配层
    * 独立读写 /sdcard/dict/NES/<rom>.sav. */
...
void rom_free(rominfo_t **rominfo)
{
   ...
   /* V1.0.53: 同上, 禁用防止把 SRAM 覆盖写回 .nes ROM 文件 (损坏 ROM). */
   // rom_savesram(*rominfo);

   /* 20260812: 共享内存池所有权修复.
    * rominfo/sram/vram 均由适配层 _my_malloc() 从共享池分配, 池由
    * nes_free_shared_memory() -> shared_mem_clear() 统一释放.
    * 这里若再用裸 free() 释放, 壁纸/菜单在 ROM 加载失败后走
    * engine_manager_unload_all() 时会二次释放同一块 -> tlsf 双重释放断言重启.
    * rom/vrom 指向适配层 osd_getromdata() 返回的 nes_rom_data, 同样不属于
    * nofrendo, 由 nes_emu_start() 失败路径统一释放. 因此本函数不再释放任何块,
    * 只负责逻辑上的"放弃", 物理释放统一交给共享池. */
   if ((*rominfo)->sram)  { /* 共享池管理, 不在此释放 */ }
   if ((*rominfo)->rom)   { /* 适配层 nes_rom_data, 不在此释放 */ }
   if ((*rominfo)->vrom)  { /* 适配层 nes_rom_data, 不在此释放 */ }
   if ((*rominfo)->vram)  { /* 共享池管理, 不在此释放 */ }
   /* rominfo 本身由共享池管理, 不在此释放 */
