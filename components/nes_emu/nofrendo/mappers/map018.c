...
{
   irq.counter = irq.latch = irq.enabled = 0;
}
...
/* 每扫描线递减一次 (nofrendo 帧循环在每条扫描线调用 intf->hblank).
 * 对齐 FCEUmm boards/18.c 的 M18IRQHook: 16-bit 计数器递减到 0 触发 IRQ
 * 并自动禁用 (霸王的大陆等 $62=98 行, 原实现 4-bit 截断导致永远黑屏). */
static void map18_hblank(int vblank)
{
   (void)vblank;
...
static void map18_write(uint32 address, uint8 value)
{
   ...
   /* IRQ latch: 4 个低半字节依次拼成 16-bit 目标值 (FCEUmm M18WriteIRQ) */
   case 0xE000: irq.latch &= 0xFFF0; irq.latch |= (value & 0x0F) << 0; break;
   case 0xE001: irq.latch &= 0xFF0F; irq.latch |= (value & 0x0F) << 4; break;
   case 0xE002: irq.latch &= 0xF0FF; irq.latch |= (value & 0x0F) << 8; break;
   case 0xE003: irq.latch &= 0x0FFF; irq.latch |= (value & 0x0F) << 12; break;
   case 0xF000: irq.counter = irq.latch; break;   /* 重载计数 */
   case 0xF001: irq.enabled = value & 0x01; break; /* 使能 (清触发由进入 IRQ 后 int_pending 自动复位) */
   case 0xF002:
      switch(value&0x03)
      {
