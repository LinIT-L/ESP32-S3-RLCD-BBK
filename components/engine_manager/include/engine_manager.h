...
typedef enum {
    ...
    ENGINE_MD,            /* MD/Genesis (Gwenesis) */
    ENGINE_SMS,           /* SMS/GG (SMSPlus) */
    ENGINE_MESHTASIC,     /* Meshtasic (旧菜单兼容保留; 本版本不再实现) */
    ENGINE_COUNT
} engine_id_t;
 ...
 * do_leak_check: true=卸载后做内存泄漏复核 (正常回主菜单场景);
 *                false=跳过 (低内存回收/周期兜底时, 此刻空闲偏低或重复卸载,
 *                          做比对会误报 [LEAK]). */
