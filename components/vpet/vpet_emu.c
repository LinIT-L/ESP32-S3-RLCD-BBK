 ...
 * E0C6200 数据区 (_RAM, 半字节), 蛋的状态(时间/饱食/清洁/好感)都在这里.
 * 经 get_mem/set_mem 经 e0c6200 统一转发, 只覆盖游戏 RAM/VRAM, 不碰系统内存. */
static uint8_t vpet_cheat_peek(uint16_t off)
{
    int v = get_mem(off);
    return (v < 0) ? 0 : (uint8_t)v;
}
static void vpet_cheat_poke(uint16_t off, uint8_t v)
{
    set_mem(off, (int)(v & 0x0F));
}
static cheater_region_t vpet_cheat_region = {
    .base = 0,
    .size = E0C6200_RAM_SIZE,
    .peek = vpet_cheat_peek,
    .poke = vpet_cheat_poke,
};

/* V1.0.9x: 是否按内容旋转 180° — 按 ROM 尺寸判断机型, 不用文件名:
 *    16KB (8192 字, 数码宝贝/Digimon 家族) → 旋转 180°
 *    12KB (6144 字, Tamagotchi P1/P2)     → 不旋转
 * V1.3.x: 16KB 家族内再细分 — Tamagotchi Angel(宠物蛋天使)/Mothra(摩斯拉) 同是
 * 16KB 但内容方向正常, 不能一刀切旋转. 由 vpet_rom_needs_rotate 判定.
 * 由 vpet_emu_start 根据读到的 words + path 设置. */
static bool s_rotate = false;

/* V1.3.x: 16KB 家族里区分旋转需求 — Digimon(数码宝贝) 家族 ROM 内容倒置 → 旋转 180°;
 * Tamagotchi Angel(宠物蛋天使) 与 Mothra(摩斯拉) 同为 16KB 但内容方向正常 → 不旋转.
 * 用文件名特征词 (大小写不敏感英文 + UTF-8 中文), 其余 16KB 保持旋转 (兼容改名 Digimon). */
static bool vpet_rom_needs_rotate(const char *path, int words) {
    if (words != 8192) return false;   /* 12KB = P1/P2 等, 不旋转 */
    if (path) {
        const char *fn = strrchr(path, '/');
        fn = fn ? fn + 1 : path;
        size_t fl = strlen(fn);
        static const char *const no_rot[4] = {
            "angel",                          /* Angel (标准英文名) */
            "\xe5\xa4\xa9\xe4\xbd\xbf",       /* 天使 (用户中文名) */
            "mothra",                         /* Mothra (标准英文名) */
            "\xe6\x91\xa9\xe6\x96\xaf\xe6\x8b\x89"  /* 摩斯拉 (用户中文名) */
        };
        for (int i = 0; i < 4; i++) {
            const char *kw = no_rot[i];
            size_t kl = strlen(kw);
            for (size_t j = 0; j + kl <= fl; j++) {
                if (strncasecmp(fn + j, kw, kl) == 0) return false;
            }
        }
    }
    return true;   /* 其余 16KB = Digimon 家族, 旋转 */
}
...
static void vpet_render(void) {
    ...
    /* 独立修改机浮标: 画浮标+检测呼出 (触发后交独立修改机任务, 本任务停走) */
    bool takeover = cheat_float_overlay_tick(s_lcd);
    if (!takeover) st7305_flush(s_lcd);
}
...
static void vpet_emu_task(void *arg) {
    ...
    while (1) {
        ...
        /* 修改机菜单打开: 冻结模拟 (独立任务跑菜单, 本引擎停走) */
        if (cheat_float_is_frozen()) {
            esp_task_wdt_reset();
            vTaskDelay(pdMS_TO_TICKS(16));
            continue;
        }

        /* (修改机由独立 cheat_float 覆盖层处理, 此处不再处理热键/浮标) */
...
esp_err_t vpet_emu_background_init(st7305_handle_t *lcd) {
    if (lcd) s_lcd = lcd;
    /* V1.0.70: tone_player 已全局初始化 (main.c), 此处不再重复初始化 */
    if (!s_rom) {
...
esp_err_t vpet_emu_start(const char *path) {
    ...
    /* V1.3.x: 16KB 家族细分 — Angel(宠物蛋天使)/Mothra(摩斯拉) 内容正常不旋转,
     * 其余 16KB (Digimon 数码宝贝家族) 内容倒置仍旋转. 文件名优先:
     * 标准名 TamagotchiAngel.bin / TamagotchiMothra.bin 或用户中文名均含特征词.
     * (V1.0.9x 的 s_rotate=(words==8192) 一刀切已由本函数取代) */
    s_rotate = vpet_rom_needs_rotate(path, words);
    ESP_LOGI(TAG, "ROM %s (%d bytes): %s", path, len, s_rotate ? "旋转180°" : "不旋转");
    ...
    s_joypad = 0xFF;
    vpet_apply_joypad(s_joypad);
    /* 独立修改机浮标: 登记本局游戏内存 region (半字节) */
    cheat_float_attach(&vpet_cheat_region, CHEAT_BITS_8);
