...
static void sys_init(const char *romdir)
{
    ...
    /* === 直接从 flash appdata 分区读取 ROM, 跳过 SD 卡 (速度快 10-20x) ===
     * 2026-08-26: 分区合并, 步步高 8.BIN/E.BIN 在 appdata 分区偏移 0/2MB.
     * subtype 0x40 (0x90 在 IDF5.5 被 tee_ota 占用, 与 partitions.csv 一致) */
    const esp_partition_t *sys_part = esp_partition_find_first(
        ESP_PARTITION_TYPE_DATA, (esp_partition_subtype_t)0x40, "appdata");
...
/* V1.1.x: 目标区可用容量上限 (字节). 与 retro_load_game 的 size 拒绝阈值 0x1e0000 保持一致.
 * 供 gam4980_emu_load 预检使用: 原 4MB 预检过宽, 1.92MB~4MB 的文件会被流式 fread
 * 越过 sys_flash 缓冲尾部写坏 PSRAM 堆, 之后才被 retro_load_game 拒绝. */
size_t gam4980_retro_rom_capacity(void)
{
    return 0x1e0000;   /* 1920KB */
}

/* ============ 修改机 (内存搜索工具) 支持接口 ============
 * 仅在"帧边界冻结"时调用 (retro_run 不在执行), 与模拟同任务, 无需加锁.
 * 只读/写引擎自身的 s_ram / sys_flash, 绝不触碰其它系统内存.
 * 由 page_modtool 经修改机 Provider 层使用. */

/* 32KB 热数据 RAM (游戏变量主战场). 未分配(引擎未加载)时返回 NULL. */
uint8_t *gam4980_modtool_ram_ptr(void)
{
    return s_ram;   /* sys_init 后 = sys_ram */
}

/* 返回热 RAM 可搜索区域大小 (始终 0x8000=32KB; 前端按安全分区再裁剪). */
size_t gam4980_modtool_ram_size(void)
{
    return 0x8000;
}

/* 读/写 32KB 热 RAM 单字节 (越界钳制, 防误改系统区). */
uint8_t gam4980_modtool_ram_peek(uint16_t off)
{
    if (!s_ram || off >= 0x8000) return 0;
    return s_ram[off];
}
void gam4980_modtool_ram_poke(uint16_t off, uint8_t v)
{
    if (!s_ram || off >= 0x8000) return;
    s_ram[off] = v;
}

/* 保存区 (sys_flash[0..0x7FFF]) — 跨局进度; 一般只读给用户看, 修改风险自担. */
uint8_t *gam4980_modtool_flash_save_ptr(void)
{
    return sys_flash;   /* 未加载时 NULL */
}

/* CPU 寄存器当前值 (帧边界读取; 供定位 PC 等高级用途). */
uint16_t gam4980_modtool_pc(void) { return s_cpu.pc; }
