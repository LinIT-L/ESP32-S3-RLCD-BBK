...
/* V1.1.1: 引擎优化设置由 os 传入 (独立于旧 menu g_menu, 旧 g_menu 在 os 构建未初始化) */
static int  s_vkey       = 2;    /* 0=关 1=开 2=自动(无手柄时开) */
static bool s_status_bar  = true; /* 游戏内状态栏 显示/隐藏 */
void gam4980_set_vkey(int vkey)      { if (vkey >= 0 && vkey <= 2) s_vkey = vkey; }
void gam4980_set_status_bar(bool on)  { s_status_bar = on; }
...
/* V1.1.x: 目标区可用容量上限 (与 retro_load_game 拒绝阈值一致, 防预检过宽越界写) */
size_t gam4980_retro_rom_capacity(void);
...
static void IRAM_ATTR render_frame(const void *data, unsigned w, unsigned h, size_t pitch) {
    ...
    if (g_display_mode != DISP_MODE_POINT2POINT) {
        ...
        if (s_status_bar) {
            svc_statusbar_draw_game(g_lcd, g_status_battery, g_status_pad_connected);
        }
        ...
        bool takeover = cheat_float_overlay_tick(g_lcd);  /* 独立修改机插件: 浮标+热键/点按呼出 */
        ...
        if (0) {
            /* V1.1.1: 点对点模式不做任何优化 (用户要求"点对点=纯2x最近邻"),
             * 原 EPX 边缘平滑路径禁用, 恒走下方纯 2x 放大, 保证像素硬朗不发虚. */
        ...
        virtual_keys_draw(g_lcd);   /* V1.0.68 fix: 补画屏幕虚拟按键 */
        if (!cheat_float_overlay_tick(g_lcd)) st7305_flush(g_lcd);  /* 独立修改机插件: 浮标+呼出, 接管则本帧不送屏 */
    }
}
...
static int16_t joypad_state(unsigned id) {
    ...
    switch (id) {
        case RETRO_DEVICE_ID_JOYPAD_A:
            ...
            phys = input_is_held(1);   /* KEY(GPIO18)=确定 → A (旧码误用 idx0=BOOT) */
            break;
        case RETRO_DEVICE_ID_JOYPAD_B:
            gp = bt_manager_is_key_pressed(F_BACK) || !(vk & 0x02) || !(wg & 0x02);
            phys = input_is_held(0);   /* BOOT(GPIO0)=返回/跳出 → B (旧码误用 idx1=KEY) */
            break;
...
int gam4980_emu_load(const char *path) {
    ...
    long sz = ftell(f);
    fseek(f, 0, SEEK_SET);
    /* V1.1.x: 上限改用目标区容量 (0x1e0000), 与 retro_load_game 拒绝阈值一致.
     * 原 4MB 预检过宽: 超容文件会被下方流式 fread 越过 sys_flash 缓冲写坏 PSRAM. */
    long max_gam = (long)gam4980_retro_rom_capacity();
    if (sz <= 0 || sz > max_gam) {
        ESP_LOGE(TAG, "文件大小异常: %ld (上限 %ld)", sz, max_gam);
        fclose(f);
        return -1;
    }
...
/* 独立修改机插件 (cheat_float+cheat_ui): 与引擎解耦的独立组件. 本引擎只登记
 * 自己可搜索的 RAM 区, 其余 (浮动图标/热键呼出/独立任务/搜索改值 UI) 全由插件承担.
 * 入口: 保留 浮动图标 + SELECT+START 热键 (插件内部处理), 不再占用本引擎任何代码. */
#define BBK_CHEAT_LO   0x0200
#define BBK_CHEAT_HI   0x8000   /* 整片 32KB 引擎 RAM */
#define BBK_CHEAT_SIZE (BBK_CHEAT_HI - BBK_CHEAT_LO)

static void gam4980_cheat_attach(void)
{
    cheater_region_t reg = {
        .base = BBK_CHEAT_LO,
        .size = BBK_CHEAT_SIZE,
        .peek = gam4980_modtool_ram_peek,
        .poke = gam4980_modtool_ram_poke,
    };
    cheat_float_attach(&reg, CHEAT_BITS_16);
}

/* 退出确认弹窗: 冻结游戏画面, 叠加统一退出确认框.
 * 已融合到统一模块 os_confirm_exit_blocking (与其它引擎外观/交互完全一致):
 * 标题"退出程序?"在上, 左"确定"右"取消", 四边黑框全画齐全;
 * 手柄 LEFT/RIGHT 黑块选择, CONFIRM=当前选中项, BACK=取消, 触摸弹窗内左=确定 右=取消.
 * 返回: GAME_EXIT_CONFIRMED=确认退出, GAME_EXIT_CANCEL=取消(恢复游戏继续). */
static game_exit_result_t gam4980_exit_confirm_dialog(void) {
    return os_confirm_exit_blocking(g_lcd) ? GAME_EXIT_CONFIRMED : GAME_EXIT_CANCEL;
}

game_exit_result_t gam4980_emu_run(void) {
    g_running = true;
    /* V1.0.68 fix: BBK 游戏不走通用 game_run_loop, 必须在此启用屏幕虚拟按键.
     * V1.1.1: 三态由 os 引擎优化设置传入 (0关/1开/2自动=无手柄时开), 不再读旧 g_menu.
     * 引擎定制: 步步高无 Select/Start 虚拟键 (B 键为 BBK 针对性 KEY_EXIT 跳出键, _joyk 已映射). */
    virtual_keys_set_hide_selsta(true);
    if (s_vkey == 1) virtual_keys_set_enabled(true);
    else if (s_vkey == 2) virtual_keys_set_enabled(!bt_manager_is_connected());
    else virtual_keys_set_enabled(false);
    gam4980_cheat_attach();   /* 登记本局游戏 RAM 给独立修改机插件 (浮动图标+热键呼出) */
    ...
    bool exit_requested = false;  /* V1.0.47: 退出前先弹确认框 */
        while (g_running) {
            ...
            uint32_t t0 = xTaskGetTickCount() * portTICK_PERIOD_MS;
            if (!cheat_float_is_frozen()) {   /* 独立修改机插件接管期间停走模拟 */
                retro_run();
                bbk_key_sound_tick();
        ...
        frame_count++;
        if (input_is_held(0)) {   /* BOOT(GPIO0) 长按退出 (旧码误用 idx1=KEY) */
            boot_hold_count++;
         ...
         *  - 软关机键(POWER_LOCK/POWER_RELEASE) 是系统级锁屏/关机动作, 不经游戏循环,
         *    不充当退出回桌面通道, 此处一律忽略.
         *  - HOME = 手柄退出到菜单(F_EXIT) 的重复路径 → 统一弹确认退出
         *  - BACK (实体返回/底部上滑) → 统一弹确认退出 */
        menu_action_t action = input_get_action();
        if (action == MENU_ACTION_HOME ||
            action == MENU_ACTION_BACK || action == MENU_ACTION_LONG_LEFT) {
            ESP_LOGI(TAG, "退出键 -> 请求退出游戏(弹确认)");
            exit_requested = true;
        }
        if (virtual_keys_is_enabled()) {
            /* 多点触控: 第一指控方向/摇杆, 第二指只控 A/B/SEL/STA (与通用模拟器一致) */
            ...
            int x1 = d1 ? pts[1].x : 0;
            int y1 = d1 ? pts[1].y : 0;
            virtual_keys_poll_multi(x0, y0, d0, x1, y1, d1);
        }
        ...
        /* 修改机呼出由独立插件 cheat_float 处理 (浮动图标 + SELECT+START 热键),
         * 引擎内不再有旧的 bbk_cheat_* 代码. */
...
void gam4980_emu_stop(void) {
    g_running = false;
    cheat_float_detach();   /* 退出本局游戏: 注销独立修改机插件的目标 RAM */
...
void gam4980_emu_unload(void) {
    ...
    g_running = false;
    cheat_float_detach();   /* 引擎卸载: 同注销修改机插件目标 */
    if (g_init_in_progress) {
