...
/* 扫描过程中每发现/更新一个 BLE 设备时的实时回调 (供上层即时刷新列表) */
static esp_hid_scan_result_cb_t s_scan_result_cb = NULL;
...
{
    esp_hid_scan_result_t *exist = find_scan_result(bda, ble_scan_results);
    if (exist) {
        /* 设备已入列表: 若此前名字未解析到而现在补到, 则补齐名字并更新 RSSI.
         * (Q36 等手柄名字常在其后广播/scan_rsp 才到, 若不补则列表永远空名) */
        exist->rssi = rssi;
        if (exist->rssi > rssi) exist->rssi = rssi;
        if ((!exist->name || exist->name[0] == '\0') && name_len && name) {
            char *name_s = (char *)malloc(name_len + 1);
            if (name_s) {
                memcpy(name_s, name, name_len);
                name_s[name_len] = 0;
                if (exist->name) {
                    free((void *)exist->name);
                }
                exist->name = (const char *)name_s;
            }
        }
        return;
    }
...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    /* 显示所有扫描到的 BLE 设备, 不限于 HID Service UUID.
     * Q36 等标准手柄广告多不含 HID_SVC(0x1812), 若过滤则列表恒为空.
     * 连接层 esp_hidh_dev_open 会再做真正的 HID 服务发现, 此处过滤只会误伤. */
    add_ble_scan_result(scan_rst->bda, scan_rst->ble_addr_type, appearance, adv_name, adv_name_len, scan_rst->rssi);

    /* 实时回调: 让上层立即刷新列表 (扫描还没结束也能看到新设备) */
    if (s_scan_result_cb) {
        esp_hid_scan_result_t *r = find_scan_result(scan_rst->bda, ble_scan_results);
        if (r) s_scan_result_cb(r);
    }
}
#endif /* CONFIG_BT_BLE_ENABLED */
...
static esp_ble_scan_params_t hid_scan_params = {
    ...
    .scan_interval          = 0x40,   /* 40ms: 50% 占空, 发现仍快且不把 CPU0 的 BTC_TASK 吃满 */
...
static void handle_ble_device_result(const struct ble_gap_disc_desc *disc)
{
    ...
    (void)fields;   /* 不按任何名字/类型屏蔽: 所有发现的 BLE 设备都加入列表(与 Bluedroid 路径一致) */
...
{
    /* 自动清理上次遗留结果, 避免 "There are old scan results" 秒失败 */
    if (bt_scan_results) { esp_hid_scan_results_free(bt_scan_results); bt_scan_results = NULL; }
    if (ble_scan_results) { esp_hid_scan_results_free(ble_scan_results); ble_scan_results = NULL; }
