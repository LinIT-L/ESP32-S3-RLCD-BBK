...
/* === 关键操作看门狗 (卡死强制重启) ===
 * 背景: 拔 TF 卡会使底层 SDMMC/FAT 读写在同步调用里永久阻塞 (如
 * usbh_msc_stop / sd_remount_vfs_from_card / sd_mount), 若无兜底 UI 会
 * 永久卡死 (仅剩后台心跳, 看似死机). 此看门狗: begin 记录操作起点, 若超过
 * timeout_ms 仍未 end, 由独立看门狗任务强制 esp_restart() 自动恢复.
 * 用法: sd_op_watchdog_begin(5000); ...阻塞调用...; sd_op_watchdog_end();
 * 调用必须在 sd_watcher_start() 之后 (看门狗随 watcher 一起跑). */
...
/* === 文曲星「忽略文件夹层级」游戏识别 (wqx) ===
 * 递归扫描 /sdcard/lava 下所有层级: 凡目录下直接含 .lav(或 Lava/ 内含 .lav) 视为一个游戏
 * (带其 LavaData), 类别套娃文件夹被穿透. */
int scan_wqx_games(char out[][64], int max_out);                 /* 扁平游戏列表(每个游戏一个名字) */
const char *wqx_game_lav_path(int idx, char *out, size_t out_size);   /* 第 idx 个游戏 .lav 完整路径 */
const char *wqx_game_data_root(int idx, char *out, size_t out_size);  /* 第 idx 个游戏数据根目录 */
void wqx_invalidate(void);                                       /* 使扫描缓存失效(进文曲星页时调用, 读到新加游戏) */
/* 分类子目录版: scope = 分类子目录(相对 /sdcard/lava), "" = 整根.
 * 供文曲星分类二级菜单使用: 每个分类只列自己内部的游戏. */
