...
电子书阅读器、蓝牙手柄与 WiFi 网页手柄,以及文曲星(LavaX)与暴龙机(虚拟宠物)引擎。
...
- **主菜单全新 UI**:原生位图图标(中央图标放大 100×100、其余统一 64×64),状态栏图标按原图 1:1 逐像素绘制(喇叭 / 数据传输 / 手柄 / 电池 / WiFi / 蓝牙)。
- **应用管理**:左侧分类栏(引擎 / 独立游戏 / 程序 / 运维)+ 右侧图标网格,单击选中、再次点击直接打开。
- **收藏系统**:各引擎收藏独立保存于 TF 卡,长按游戏名 2 秒收藏/取消;打开收藏栏自动清理失效(已改名/删除)项。
- **阅读器**:进入先在书架选书;支持 TXT / FB2 / EPUB;内置仿宋/黑体简体点阵字库(16/24/32 三档),四向旋转、夜间模式、进度与书签。
- **列表交互**:进入游戏/书籍列表默认显示收藏栏,空收藏时显示「暂无收藏」引导,不再误报「无法定位游戏」;引擎根目录无游戏时自动隐藏「全部」侧栏项。
- **稳定性**:阅读器二次进入路径、应用管理弹窗点击穿透、收藏栏空态等交互问题修复。
...
**应用**
- 电子书阅读器:txt/GBK/UTF-8,书签/进度,4 方向旋转,夜间模式,内置**仿宋/黑体**简体点阵字库(16/24/32 三档字号)
...
- **电子书**:书库选书;阅读时点上半=上一页、下半=下一页、中间=设置菜单;书设置含旋转(上下左右)、夜间模式、**字体(仿宋/黑体)**、字号(16/24/32)、边距
...
- 内置 **GB2312 简体点阵字库**(无繁体),全部为低分辨率优化的原生点阵,非矢量缩放
- **两种字体**:仿宋(笔画最细,经典小说阅读风格)、黑体(与主菜单同一款 24px 点阵字体)
- **三档字号**:16 / 24 / 32 px,数字/字母随字号同步放大;在书设置里切换即时生效(分页不重排,阅读进度不受影响)
- 常用标点已用同尺寸国标点阵补齐;主菜单字体与阅读器"黑体"为同一字库,生僻字(GBK 扩展)保留
- 默认:仿宋 24px
