...
# 输出在 dist/ 目录下:
#   - LinTOS_Vx.y.z_merged_YYYYMMDD.zip   单文件 16MB 镜像, 烧到 0x0
#   - LinTOS_Vx.y.z_flashdl_YYYYMMDD.zip  4 文件, 配 Espressif Flash Download Tool
#                                                     或 esptool.py 多地址烧录
...
# ============ 打包 1: 合并镜像 (单文件) ============
MERGED_DIR="${DIST_DIR}/LinTOS_${VERSION}_merged_${DATE}"
rm -rf "${MERGED_DIR}"
mkdir -p "${MERGED_DIR}"
cp "${MERGED_BIN}" "${MERGED_DIR}/LinTOS_16mb.bin"

# 烧录说明
cat > "${MERGED_DIR}/README.txt" <<EOF
LinTOS 合并固件镜像 ${VERSION} (${DATE})
=========================================
...
文件清单
--------
- LinTOS_16mb.bin   完整 16MB Flash 镜像
                              bootloader         @ 0x000000  (32KB)
                              partition-table    @ 0x008000  (3KB)
                              nvs                @ 0x009000  (24KB)
                              app (LinTOS)@ 0x010000  (~1.5MB)
...
-------------------------------------------------
  ...
  Flash Size: 16MB
  |------------------------|
  | [x] | LinTOS_16mb.bin | 0x0 |
  |------------------------|
  点击 START 开始烧录.
  烧完后按一下 RST 按钮重启设备.
EOF

MERGED_ZIP="${DIST_DIR}/LinTOS_${VERSION}_merged_${DATE}.zip"
cd "${DIST_DIR}"
rm -f "${MERGED_ZIP}"
...
FLASH_DIR="${DIST_DIR}/LinTOS_${VERSION}_flashdl_${DATE}"
rm -rf "${FLASH_DIR}"
mkdir -p "${FLASH_DIR}"
cp "${BL_BIN}"  "${FLASH_DIR}/"
cp "${PT_BIN}"  "${FLASH_DIR}/"
...
FLASH_ZIP="${DIST_DIR}/LinTOS_${VERSION}_flashdl_${DATE}.zip"
cd "${DIST_DIR}"
rm -f "${FLASH_ZIP}"
